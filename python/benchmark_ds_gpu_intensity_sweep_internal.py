...
def ds_recurrence_kernel(x, y, k: int):
...
def benchmark_one(n, k, iters):
    ...
    # Keep values bounded so the recurrence stays numerically stable.
    ...
    t_ds = time_fn(ds_recurrence_kernel, x64, y64, k, iters=iters)

    # Accuracy spot-check on a small prefix against f64
    ref = np.asarray(block(f64_kernel(x64[:16], y64[:16])))
    out_ds = np.asarray(block(ds_recurrence_kernel(x64[:16], y64[:16], k)))
