...
inline double ds_to_double(float hi, float lo) {
  return static_cast<double>(hi) + static_cast<double>(lo);
}

// Input:  x shape [N], y shape [N] as f64
// Output: out shape [N] as f64
ffi::Error DsFusedAddMulToF64Impl(ffi::Buffer<ffi::F64> x,
  ...
  if (x_dims.size() != 1 || y_dims.size() != 1) {
    return ffi::Error::InvalidArgument(
        "ds.fused_add_mul_to_f64 expects rank-1 inputs");
  }
  ...
  for (int64_t i = 0; i < n; ++i) {
    ...
    op[i] = ds_to_double(prod_hi, prod_lo);
  }
